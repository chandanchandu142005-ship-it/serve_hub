...
(() => {
  ...
  const I18N = {
    en: { hero: 'Home services, booked in seconds.', book: 'Book Now', search: 'Search', login: 'Log in', join: 'Join as professional', selectLang: 'Select Language' },
    hi: { hero: 'घर की सेवाएँ, सेकंडों में बुक करें।', book: 'अभी बुक करें', search: 'खोजें', login: 'लॉग इन', join: 'प्रोफेशनल बनें', selectLang: 'भाषा चुनें' },
    kn: { hero: 'ಮನೆ ಸೇವೆಗಳು, ಸೆಕೆಂಡುಗಳಲ್ಲಿ ಬುಕ್ ಮಾಡಿ.', book: 'ಈಗಲೇ ಬುಕ್ ಮಾಡಿ', search: 'ಹುಡುಕಿ', login: 'ಲಾಗಿನ್ ಮಾಡಿ', join: 'ವೃತ್ತಿಪರರಾಗಿ ಸೇರಿ', selectLang: 'ಭಾಷೆಯನ್ನು ಆಯ್ಕೆಮಾಡಿ' },
    te: { hero: 'ఇంటి సేవలు, సెకన్లలో బుక్ చేయండి.', book: 'ఇప్పుడే బుక్ చేయండి', search: 'శోధించండి', login: 'లాగిన్', join: 'ప్రొఫెషనల్‌గా చేరండి', selectLang: 'భాషను ఎంచుకోండి' },
    ta: { hero: 'வீட்டு சேவைகள், நொடிகளில் முன்பதிவு செய்யுங்கள்.', book: 'இப்போதே பதிவு செய்', search: 'தேடு', login: 'உள்நுழைக', join: 'தொழில்முறையாக இணையுங்கள்', selectLang: 'மொழியைத் தேர்ந்தெடுக்கவும்' },
    bn: { hero: 'হোম সার্ভিস, সেকেন্ডে বুক করুন।', book: 'এখনই বুক করুন', search: 'সন্ধান করুন', login: 'লগ ইন', join: 'পেশাদার হিসাবে যোগ দিন', selectLang: 'ভাষা নির্বাচন করুন' },
    mr: { hero: 'घरगुती सेवा, सेकंदात बुक करा.', book: 'आत्ताच बुक करा', search: 'शोधा', login: 'लॉगिन करा', join: 'व्यावसायिक म्हणून सामील व्हा', selectLang: 'भाषा निवडा' },
    gu: { hero: 'ઘરની સેવાઓ, સેકન્ડોમાં બુક કરો.', book: 'હમણાં બુક કરો', search: 'શોધો', login: 'લૉગિન', join: 'પ્રોફેશનલ તરીકે જોડાઓ', selectLang: 'ભાષા પસંદ કરો' },
    ml: { hero: 'വീട്ടുസേവനങ്ങൾ, സെക്കൻഡുകൾക്കുള്ളിൽ ബുക്ക് ചെയ്യാം.', book: 'ഇപ്പോൾ ബുക്ക് ചെയ്യുക', search: 'തിരയുക', login: 'ലോഗിൻ', join: 'പ്രൊഫഷണലായി ചേരുക', selectLang: 'ഭാഷ തിരഞ്ഞെടുക്കുക' },
    pa: { hero: 'ਘਰ ਦੀਆਂ ਸੇਵਾਵਾਂ, ਸਕਿੰਟਾਂ ਵਿੱਚ ਬੁੱਕ ਕਰੋ।', book: 'ਹੁਣੇ ਬੁੱਕ ਕਰੋ', search: 'ਖੋਜੋ', login: 'ਲੌਗਇਨ', join: 'ਪੇਸ਼ੇਵਰ ਵਜੋਂ ਸ਼ਾਮਲ ਹੋਵੋ', selectLang: 'ਭਾਸ਼ਾ ਚੁਣੋ' },
    es: { hero: 'Servicios para el hogar, reservados en segundos.', book: 'Reservar ahora', search: 'Buscar', login: 'Iniciar sesión', join: 'Únete como profesional', selectLang: 'Seleccionar idioma' },
    fr: { hero: 'Services à domicile, réservés en quelques secondes.', book: 'Réserver maintenant', search: 'Rechercher', login: 'Se connecter', join: 'Devenir professionnel', selectLang: 'Choisir la langue' },
    ar: { hero: 'خدمات منزلية، محجوزة في ثوانٍ.', book: 'احجز الآن', search: 'بحث', login: 'تسجيل الدخول', join: 'انضم كمهني', selectLang: 'اختر اللغة' },
  };

  const t = k => { const d = I18N[Store.state.lang] || I18N.en; return d[k] || I18N.en[k]; };

  const getActiveLang = () => LANGUAGES.find(l => l.code === Store.state.lang) || LANGUAGES[0];

  const openLanguageModal = () => {
    const activeCode = Store.state.lang || 'en';
    const html = `
      <div style="text-align:center;padding:4px 0">
        <div style="width:52px;height:52px;background:var(--primary-50);color:var(--primary);border-radius:50%;display:inline-flex;align-items:center;justify-content:center;margin-bottom:12px">
          ${icon('globe', 26)}
        </div>
        <h3 style="font-size:22px;font-weight:900;margin-bottom:6px;color:var(--ink)">Select Your Language</h3>
        <p style="font-size:13.5px;color:var(--ink-3);margin-bottom:18px">Choose from 13 supported regional and global languages</p>
        
        <div class="lang-column-list">
          ${LANGUAGES.map(l => `
            <button class="lang-column-item ${activeCode === l.code ? 'active' : ''}" data-act="set-lang" data-code="${l.code}">
              <div class="lang-item-left">
                <span class="lang-flag-badge">${esc(l.flag)}</span>
                <div class="lang-title-group">
                  <span class="lang-native-title">${esc(l.native)}</span>
                  <span class="lang-sub-title">(${esc(l.name)})</span>
                </div>
              </div>
              <span class="lang-check-ic">${activeCode === l.code ? icon('check', 18) : ''}</span>
            </button>
          `).join('')}
        </div>
      </div>
    `;
    openModal(html, { maxWidth: '540px' });
  };
    ...
    <header class="navbar" id="navbar">
      <div class="nav-inner">
        ...
        <nav class="nav-links" aria-label="Main">
          ...
          <a class="nav-link" href="#/categories">Services ${icon('chevronDown', 14)}</a>
        ...
        <div class="nav-actions">
          ...
          <button class="btn btn-soft btn-sm" data-act="lang" aria-label="Select Language" title="Select Language" style="gap:6px;padding:6px 12px">
            ${icon('globe', 15)}<span style="font-weight:800;font-size:13px">${curLang.native}</span>
          </button>
      ...
      <div class="mobile-menu" id="mobile-menu">
        ...
        <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--line)">
          <div style="font-size:12px;font-weight:700;color:var(--ink-4);text-transform:uppercase;margin-bottom:8px">Select Language / ਭਾਸ਼ਾ / ภาษา</div>
          <div class="lang-row-grid" style="justify-content:flex-start;gap:6px">
            ${LANGUAGES.map(l => `
              <button class="lang-chip ${Store.state.lang === l.code ? 'active' : ''}" data-act="set-lang" data-code="${l.code}" style="padding:6px 12px;font-size:12px">
                <span>${l.flag}</span>
                <span>${l.native}</span>
              </button>
            `).join('')}
  ...
  const openLocationModal = () => {
    ...
    U.$('#loc-detect').addEventListener('click', async () => {
      const status = U.$('#loc-status');
      const btn = U.$('#loc-detect');
      
      btn.disabled = true;
      btn.innerHTML = `${icon('timer', 14)} Detecting…`;
      if (status) status.innerHTML = `<div style="display:flex;align-items:center;gap:8px;padding:10px 12px;background:var(--primary-50);border-radius:10px;margin-top:10px;font-size:12.5px;color:var(--primary-700);font-weight:600">${icon('navigation', 14)} Detecting your GPS position &amp; address…</div>`;

      const setLocationAndClose = (cityName, areaName) => {
        const cleanCity = cityName || 'Hyderabad';
        const cleanArea = (areaName && areaName !== cleanCity) ? areaName : '';
        Store.setLocation({ city: cleanCity, area: cleanArea });
        closeModal();
        toast(`📍 Location set to ${Store.currentLocation().label}`, 'success');
      ...
      const fallbackIpGeo = async () => {
        try {
          ...
          if (d && (d.city || d.region)) {
            setLocationAndClose(d.city || d.region, d.sublocality || d.city);
            return true;
          }
        } catch (e) {}
        try {
          ...
          const d = await res.json();
          if (d && d.city) {
            setLocationAndClose(d.city, d.regionName || '');
            return true;
          }
        } catch (e) {}
        return false;
      ...
      if (!('geolocation' in navigator)) {
        ...
        if (!ok) {
          btn.disabled = false;
          btn.innerHTML = `${icon('navigation', 14)} Use my location`;
          if (status) status.innerHTML = `<span class="small" style="color:var(--warn-600)">${icon('alert', 13)} Geolocation is not supported by your browser. Please type your area above.</span>`;
        }
        return;
      }
      ...
      navigator.geolocation.getCurrentPosition(
        async pos => {
          ...
          // 1. Attempt Nominatim reverse geocode (highest precision)
          try {
            const nomRes = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&addressdetails=1`, {
              headers: { 'Accept-Language': 'en' }
            });
            if (nomRes.ok) {
              const nomData = await nomRes.json();
              const addr = nomData.address || {};
              detectedArea = addr.suburb || addr.neighbourhood || addr.residential || addr.subdistrict || addr.quarter || addr.village || addr.road || '';
              detectedCity = addr.city || addr.town || addr.county || addr.state_district || addr.state || '';
            }
          } catch (err) {}

          // 2. Fallback to BigDataCloud reverse geocode if needed
          if (!detectedCity) {
            try {
              ...
              if (bdcRes.ok) {
                ...
                detectedArea = bdcData.locality || '';
              }
            } catch (err) {}
          }

          if (detectedCity || detectedArea) {
            setLocationAndClose(detectedCity || 'Hyderabad', detectedArea);
          } else {
            const ok = await fallbackIpGeo();
            if (!ok) {
              setLocationAndClose('Hyderabad', 'Banjara Hills');
            }
          }
        },
        async err => {
          // Geolocation error or permission denied -> Try IP-based location fallback before failing
          const ok = await fallbackIpGeo();
          if (!ok) {
            btn.disabled = false;
            btn.innerHTML = `${icon('navigation', 14)} Use my location`;
            if (status) {
              const msg = err.code === 1
                ? 'Location permission was denied. You can select your city or type your area above.'
                : 'Could not fetch your GPS location. Please choose a city below or type your area above.';
              status.innerHTML = `<span class="small" style="color:var(--warn-600)">${icon('alert', 13)} ${msg}</span>`;
            }
          }
        },
  ...
  const installModalBody = () => `
    <div class="install-head"><span class="install-logo">${icon('zap', 22)}</span><div><b style="font-size:15px">Servehub App</b><div class="small muted">Home services, one tap away</div></div></div>
    <p class="small muted" style="margin:12px 0 14px">Install Servehub on ${isIOS ? 'your iPhone / iPad' : isAndroid ? 'your Android phone' : 'this computer'} — opens full-screen, works offline and feels just like a native app.</p>

    ${window.SH_PWA && SH_PWA.canInstall() ? `
    <div style="margin-bottom:16px">
      <button class="btn btn-primary" data-act="trigger-install" style="width:100%;padding:12px;font-size:14px;font-weight:700;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:var(--sh-md)">
        ${icon('download', 18)} Install Servehub App Now
      </button>
    </div>` : ''}
    ...
    <div class="install-steps">
      ${isIOS
        ...
        : isAndroid
          ? `<div class="install-step"><span class="is-n">1</span><div><b>Open in Chrome Mobile</b><div class="small muted">or use Chrome browser menu</div></div></div>
             <div class="install-step"><span class="is-n">2</span><div><b>Tap the ⋮ menu</b><div class="small muted">top-right corner of Chrome</div></div></div>
             ...
             <button class="btn btn-soft btn-sm" data-act="android-app" style="width:100%;margin-top:8px">${icon('android', 14)} Prefer an app file? Get the Android APK</button>`
          : isEdge
            ? `<div class="install-step"><span class="is-n">1</span><div><b>Click the ⊕ Install icon in the address bar</b><div class="small muted">top right of your Edge address bar</div></div></div>
               <div class="install-step"><span class="is-n">2</span><div><b>Or click Edge … Menu ➔ Apps ➔ Install Servehub</b><div class="small muted">top right corner of Microsoft Edge</div></div></div>
               <div class="install-step"><span class="is-n">3</span><div><b>Click "Install"</b><div class="small muted">Servehub opens instantly as a Desktop App! 🎉</div></div></div>`
            : `<div class="install-step"><span class="is-n">1</span><div><b>Click Chrome ⋮ Menu ➔ Save and share ➔ Install Servehub</b><div class="small muted">or click the ⊕ install icon in the address bar</div></div></div>
               <div class="install-step"><span class="is-n">2</span><div><b>Click "Install"</b><div class="small muted">Servehub opens in its own standalone desktop app window — done 🎉</div></div></div>`}
    </div>`;
  ...
  const renderRoute = () => {
    ...
    if (r && ['book', 'track', 'invoice'].includes(r.name) && !Store.isLoggedIn()) {
      ...
      toast('Please log in to book a service 🔐', 'info');
      out = Auth.render('login') || { html: '', wire: null };
      out.lay = 'auth';
      location.hash = '#/login';
    }
  ...
  const actions = {
    ...
    'set-lang': el => {
      ...
      toast(`Language changed to ${langObj.native} (${langObj.name}) 🎉`, 'success');
      renderRoute();
    },
    ...
    'pw-toggle': el => {
      ...
      el.innerHTML = icon(isPw ? 'eyeOff' : 'eye', 16);
      el.setAttribute('aria-label', isPw ? 'Hide password' : 'Show password');
    ...
    'install-app': async el => {
      if (window.SH_PWA && SH_PWA.canInstall()) {
        ...
        if (ok) {
          toast('Servehub App installed successfully! 🎉', 'success');
          const bar = U.$('.pwa-bar'); bar && bar.remove();
          return;
        }
      }
      if (window.SH_PWA && SH_PWA.isStandalone()) {
        toast('Servehub is already installed on this device 🎉', 'success');
        return;
      }
      openModal(modalShell('Get the Servehub app', installModalBody(), `<button class="btn btn-ghost" data-act="close-modal">Maybe later</button><button class="btn btn-primary" data-act="trigger-install">${icon('download', 14)} Install Now</button>`));
    },
    'trigger-install': async () => {
      ...
      if (window.SH_PWA && SH_PWA.isStandalone()) {
        toast('Servehub is already running as a standalone app 🎉', 'success');
        return;
      }

      // 2. If running on HTTP IP (e.g. 10.159.199.108), try switching to localhost where native PWA install is enabled by Chrome/Edge
      if (location.hostname !== 'localhost' && location.hostname !== '127.0.0.1' && location.protocol !== 'https:') {
        const localUrl = `http://localhost:4000/${location.hash}`;
        try {
          await fetch('http://localhost:4000/api/health', { mode: 'no-cors' });
          toast('Opening localhost secure origin for native PWA installation… 🚀', 'info');
          setTimeout(() => { window.location.href = localUrl; }, 400);
          return;
        } catch (e) {}
      }

      // 3. Open ServeHub as an independent, chromeless Standalone Desktop App Window!
      const appWin = window.open(
        window.location.href,
        'ServehubAppWindow',
        'popup=1,width=1280,height=840,menubar=no,toolbar=no,location=no,status=no,resizable=yes,scrollbars=yes'
      );

      if (appWin) {
        toast('Servehub App launched in standalone desktop window! 🚀', 'success');
      } else {
        toast('Please open http://localhost:4000 to install as a native PWA app', 'info');
      }
    },
