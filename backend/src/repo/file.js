...
module.exports = {
  ...
  /* ---------- users ---------- */
  async findUserByEmail(email) { const em = (email || '').toLowerCase(); return load().users.find(u => (u.email || '').toLowerCase() === em) || null; },
  async findUserByGoogleId(googleId) { return load().users.find(u => u.googleId === googleId) || null; },
  async findUserById(id) { return load().users.find(u => u.id === id) || null; },
  async createUser(o) {
    const d = load();
    const u = {
      id: d.seq.user++, name: o.name, email: (o.email || '').toLowerCase(), phone: o.phone || '',
      googleId: o.googleId || '', avatar: o.avatar || '',
      role: o.role || 'customer', passwordHash: o.passwordHash || '',
      walletBalance: 250, rewardPoints: 120, emailVerified: true, createdAt: now(),
    };
    d.users.push(u); save();
    return clone(u);
  },
  ...
  async updateUserPassword(email, passwordHash) {
    ...
    const u = d.users.find(x => (x.email || '').toLowerCase() === em);
