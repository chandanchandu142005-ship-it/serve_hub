...
module.exports = {
  ...
  /* ---------- users ---------- */
  async findUserByEmail(email) { return clean(await M.User.findOne({ email }).lean()); },
  ...
  async updateUserPassword(email, passwordHash) {
    await M.User.updateOne({ email }, { $set: { passwordHash } });
    return clean(await M.User.findOne({ email }).lean());
  },
