...
const SMTP_HOST = (process.env.SMTP_HOST || process.env.EMAIL_HOST || '').trim();
...
const SMTP_PASS = (process.env.SMTP_PASS || process.env.EMAIL_PASSWORD || '').trim();
const MAIL_FROM = process.env.MAIL_FROM || process.env.EMAIL_FROM || 'Servehub <no-reply@servehub.in>';
...
if (!isDemo()) {
  ...
  transporter = nodemailer.createTransport({
    ...
    auth: SMTP_USER ? { user: SMTP_USER, pass: cleanPass } : undefined,
