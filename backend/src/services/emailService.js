...
function getTransporter() {
  if (!transporter && isRealSmtpConfigured()) {
    ...
    transporter = nodemailer.createTransport({
      ...
      auth: { user: EMAIL_USER, pass: cleanPassword },
      tls: {
...
/**
 * Generate HTML body for Registration OTP email.
 */
function buildRegistrationEmailHtml(otp, userName = 'User') {
  return `
  <!DOCTYPE html>
  <html>
  <head>
    <meta charset="utf-8">
    <title>ServeHub Email Verification Code</title>
  </head>
  <body style="font-family: 'Inter', Arial, sans-serif; background-color: #f4f6f9; margin: 0; padding: 20px;">
    <div style="max-width: 520px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e5e7eb;">
      ...
      <div style="background: linear-gradient(135deg, #2563eb, #1d4ed8); padding: 30px 32px; text-align: left;">
        ...
        <p style="color: #dbeafe; font-size: 14px; margin: 6px 0 0 0;">Account Registration Verification</p>
      </div>

      <!-- Content -->
      <div style="padding: 32px;">
        <p style="font-size: 16px; color: #1f2937; margin-top: 0;">Welcome, ${userName}! 👋</p>
        
        <p style="font-size: 15px; color: #4b5563; line-height: 1.6;">
          Thank you for creating an account with ServeHub. Please verify your email address to complete registration.
        </p>
...
async function sendPasswordResetOTP({ to, otp, userName }) {
  ...
  if (!isRealSmtpConfigured()) {
    ...
    console.log(`Note:       To receive real emails in your inbox, set your EMAIL_USER and EMAIL_PASSWORD in backend/.env`);
    console.log(`======================================================\n`);
 ...
 */
async function sendRegistrationOTP({ to, otp, userName }) {
  const subject = 'ServeHub Email Verification Code';
  const html = buildRegistrationEmailHtml(otp, userName);

  if (!isRealSmtpConfigured()) {
    console.log(`\n======================================================`);
    console.log(`⚡ SERVEHUB REGISTRATION OTP`);
    console.log(`To Email:   ${to}`);
    console.log(`OTP Code:   ${otp}`);
  ...
  try {
    ...
    console.log(`[EMAIL SERVICE] Registration OTP email sent to ${to} (MessageId: ${info.messageId})`);
    return { delivered: true, demo: false, messageId: info.messageId };
  } catch (error) {
    console.error(`[EMAIL SERVICE WARNING] SMTP Delivery failed to ${to}:`, error.message);
    console.log(`\n======================================================`);
    console.log(`⚡ SERVEHUB REGISTRATION OTP (FALLBACK DISPLAY)`);
    console.log(`To Email:   ${to}`);
    console.log(`OTP Code:   ${otp}`);
    console.log(`======================================================\n`);
    return { delivered: false, demo: true, otp, error: error.message };
