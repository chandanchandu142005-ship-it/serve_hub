  ...
  const wireStep = root => {
    ...
    if (detectLocBtn) {
      detectLocBtn.addEventListener('click', async () => {
        const status = U.$('#bk-loc-status', root);
        detectLocBtn.disabled = true;
        detectLocBtn.innerHTML = `${icon('timer', 16)} Detecting location…`;
            ...
            if (res.ok) {
              ...
              if (data && data.latitude && data.longitude) {
                const newAddr = {
                  ...
                  line: [data.city, data.region].filter(Boolean).join(', '),
                  area: data.city || 'Locality',
                  city: data.city || 'City',
                  pin: data.postal || '400050',
                  primary: true,
                };
            ...
            if (res.ok) {
              ...
              if (data && data.lat && data.lon) {
                const newAddr = {
                  id: 'a_gps_' + Date.now(),
                  label: 'Exact GPS Location',
                  line: [data.city, data.regionName].filter(Boolean).join(', '),
                  area: data.city || 'Locality',
                  city: data.city || 'City',
                  pin: data.zip || '400050',
                  primary: true,
          ...
          async pos => {
            ...
            try {
              ...
              if (res.ok) {
                ...
                line = [data.locality, data.city].filter(Boolean).join(', ') || line;
                pin = data.postcode || pin;
              }
            } catch (e) {}
            ...
            const newAddr = {
              id: 'a_gps_' + Date.now(),
              label: 'Exact GPS Location',
              line: `Lat: ${latitude.toFixed(5)}, Lng: ${longitude.toFixed(5)}, ${line}`,
              area: city,
              city: city,
              pin: pin,
              primary: true,
            ...
            saveBk();
            Store.persist();
            toast('📍 Exact location detected!', 'success');
            window.App && App.refresh();
          },
          async err => {
            const ok = await fallbackIpGeo();
            ...
            if (status) status.innerHTML = `<span style="color:var(--warn-600)">Location permission denied. Please select from saved addresses below.</span>`;
            toast('Location permission required. Select saved address below.', 'warn');
          },
