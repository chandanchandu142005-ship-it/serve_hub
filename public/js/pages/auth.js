    ...
    form?.addEventListener('submit', async e => {
      ...
      try {
        ...
        if (res && res.success && res.token && res.user) {
          ...
          const errMsg = res?.error || res?.message || 'Login failed. Please check your credentials.';
          toast(errMsg, 'error');
        }
      } catch (err) {
        if (submitBtn) {
        ...
        toast('Unable to connect to server. Please check your internet connection and try again.', 'error');
      }
    });
  };
    ...
    form?.addEventListener('submit', async e => {
      ...
      try {
        ...
        if (res && res.success && res.token && res.user) {
          ...
          toast(res?.error || res?.message || 'Registration failed.', 'error');
        }
      } catch (err) {
        if (submitBtn) {
          submitBtn.disabled = false;
